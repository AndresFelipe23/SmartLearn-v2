<template>
  <div>
    ...
    <div class="div" v-if="infoUsuario.rol == '6525b7b49b07590f3ba23626'">
    ...
    <div class="div" v-if="infoUsuario.rol == '6525b7bc9b07590f3ba23629'">

      <v-row style=" width:120%; ">
        <v-col cols="12" sm="3" class="fondosub" v-for="(i, index) in pruebamil" :key="index">
          <v-badge color="pink" :content="i.Actividades.length">
            <v-menu bottom origin="center center" transition="scale-transition">
              <template v-slot:activator="{ on, attrs }">
                <label v-bind="attrs" v-on="on">
                  {{ i._id.name }}
      ...
      <vs-dialog blur prevent-close v-model="active" class="dialogo">
        <v-row class="  text-capitalize " style="font-weight: bold;font-size:30px;color:rgb(6, 90, 187);">
          ...
          <v-col cols="12" sm="5" style="float:left"><b-icon icon="bell-fill" font-scale="1.5"
              style="color:rgb(6, 90, 187);"></b-icon> Actividades</v-col>
          <v-col cols="12" sm="2">

            <v-select style="margin-top:-10px;" :items="items" v-model="search" label="filtro" single-line></v-select>
          </v-col>
            ...
            <v-tooltip bottom color="green">
              <template v-slot:activator="{ on, attrs }">
                <button @click="activador = !activador">
                  <img v-bind="attrs" v-on="on" src="../../assets/Icono.png" alt="icono"
                    style="max-height:35px;max-width:40px;">
                </button>
              </template>
        ...
        <div style="max-height:350px; overflow: scroll;" class="yerlis">
          <div v-for="(dato, index) in searching" :key="index">
            <div style="background-color:#D4D4D4;height:115px;padding:4px;border-radius:10px;margin:10px">
              ...
              <div
                style="height:80px;float:right; width: 70px; display: flex;justify-content: center;align-items: center;">
                <button> <b-icon v-if="dato.tiempoRestante > 0" @click="cambiarEnvioTarea(index)" style="color:green;"
                    icon="telegram" font-scale="3"></b-icon></button>
                <button> <b-icon v-if="dato.tiempoRestante <= 0" style="color:gray;" icon="telegram"
                    font-scale="3"></b-icon></button>
              </div>
      ...
      <vs-dialog blur prevent-close v-model="activado" class="dialogo">
        <v-row class="  text-capitalize " style="font-weight: bold;font-size:30px;color:rgb(6, 90, 187);">
          ...
          <v-col cols="12" sm="8" style="float:left"><b-icon icon="bell-fill" font-scale="1.5"
              style="color:rgb(6, 90, 187);"></b-icon> Micro Clases</v-col>
        ...
        <div style="min-height:350px; overflow: scroll;" class="yerlis">
          <div v-for="(cla, index) in clasesEstudiante" :key="index">
            <div v-for="(claEs, i) in cla" :key="i"
              style="background-color:#D4D4D4;padding:4px;border-radius:10px;margin:10px">
...
export default {
  ...
  async mounted() {
    ...
    if (this.infoUsuario.rol == '6525b7bc9b07590f3ba23629') {
